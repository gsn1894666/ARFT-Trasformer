...
# 定义 device
cuda = not no_cuda and torch.cuda.is_available()  # 再使用 no_cuda
...
def visualize_features(model, src_loader, tgt_loader, iteration):
    ...
    with torch.no_grad():
        for src_data,  _ in src_loader:
            ...
            src_feats, _ = model(src_data, return_features=True)  # 只取 features
            src_features.append(src_feats.cpu().numpy())

        for tgt_data, _ in tgt_loader:
            if cuda:
                tgt_data = tgt_data.cuda(),
            tgt_feats, _ = model(tgt_data, return_features=True)  # 只取 features
...
class TabNetClassifier(nn.Module):
    ...
    def forward(self, x, return_features=False):
        ...
        if return_features:
            return pred, features  # 返回分类结果 + 提取的特征
        return pred
...
def train(model):
    ...
    lambda_mmd = 0.5  # 控制 MMD 影响力的权重
    # 原始数据记录
    total_loss_history = []
    test_loss_history = []  # 记录测试损失
    test_iterations = []  # 记录测试轮数
    iteration_history = []
    # 每 100 次的平均值记录
    avg_loss_history = []
    precision_history = []
    ...
    for i in range(1, iteration + 1):
        ...
        try:
            tgt_data, _ = next(tgt_iter)  # 只需要目标域的数据，不需要标签
        except StopIteration:
            tgt_iter = iter(tgt_loader)
...
def test_source(model):
    ...
    print(f"源域 Precision: {precision:.4f}, Recall: {recall:.4f}, Balance: {balance:.4f}, F1: {f1:.4f}")
    return accuracy
...
def test(model):
    ...
    avg_test_loss = test_loss / len(tgt_loader)  # 计算平均测试损失
    precision = precision_score(all_labels, all_preds, average='binary', zero_division=1)
    recall = recall_score(all_labels, all_preds, average='binary', zero_division=1)
    f1 = f1_score(all_labels, all_preds, average='binary', zero_division=1)
    ...
    print("预测标签分布:", np.bincount(all_preds))
...
if __name__ == '__main__':
    ...
    # 动态获取特征数
    src_sample_data = src_loader.dataset[0][0]  # 获取第一个样本的特征数据
    feature_dim = src_sample_data.shape[0]  # 假设特征在第 0 维
    ...
    print(f"tgt训练集样本数: {len(src_train_dataset)}")
